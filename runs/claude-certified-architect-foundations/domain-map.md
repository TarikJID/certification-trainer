...
- Official exam guide PDF (fetched and archived): https://everpath-course-content.s3-accelerate.amazonaws.com/instructor%2F6nizmqk8tpzpfjvt6qmmav7rh%2Fpublic%2F1783542750%2FClaude+Certified+Architect+%E2%80%93+Foundations+Exam+Guide.pdf
- Archived copy (full text extraction, all 18 sections, used as the source of truth for this map): `runs/claude-certified-architect-foundations/source-exam-guide.txt`

## Notes on sourcing

- The certification's public landing page (Skilljar) does not itself enumerate domains, weightings, or task statements — it only names the four "core technology areas" (Claude Code, Claude Agent SDK, Claude API, MCP) and links out to a downloadable Exam Guide PDF. That PDF is the actual blueprint document and is the source used for everything below.
- The PDF could not be read directly by the file tool as a URL; it was fetched via WebFetch (which surfaced the binary as a cached local file), then re-read with the Read tool's native PDF handling to extract full per-page text across all 39 pages. That extraction is what was archived to `source-exam-guide.txt` and used to build this map.
- Section 6 ("Detailed Objectives by Domain") is an explicit, formally structured enumeration: each domain has a bare-numbered "Task Statement X.Y: <title>" line, each followed by a "Knowledge of:" bullet list and a "Skills in:" bullet list. This matches the checklist's expected shape exactly, so no prose-extraction fallback was needed — the task statements and bullets below are reproduced verbatim from that section.
- The exam blueprint (Section 4) gives domain weightings as percentages of scored items; these are carried into the `Domain:` lines below.
- No ambiguity was found about which exam this material describes: the PDF's title page, exam code (CCAR-F), and version/date match the certification named in this run's instructions exactly.
- Bullet counts below were tallied directly against the archived text, domain by domain and task by task; totals are reported in the summary at the end.

---

- Domain: Agentic Architecture & Orchestration (27%)
  Description: Covers designing and implementing agentic loops, orchestrating coordinator/subagent multi-agent systems, configuring subagent context passing and spawning, enforcing multi-step workflows via hooks and prerequisites, decomposing complex tasks, and managing session state/resumption/forking.
  Task statements:
  ...
  Bullet count for this domain: 48 (Task 1.1: 6, 1.2: 8, 1.3: 8, 1.4: 6, 1.5: 6, 1.6: 6, 1.7: 8)

- Domain: Tool Design & MCP Integration (18%)
  Description: Covers designing clear/effective tool interfaces, implementing structured MCP error responses, distributing tools across agents and configuring tool_choice, integrating MCP servers into Claude Code and agent workflows, and selecting built-in tools (Read, Write, Edit, Bash, Grep, Glob) effectively.
  Task statements:
  ...
  Bullet count for this domain: 43 (Task 2.1: 8, 2.2: 8, 2.3: 9, 2.4: 9, 2.5: 9)

- Domain: Claude Code Configuration & Workflows (20%)
  Description: Covers configuring CLAUDE.md hierarchy/scoping/modular organization, creating custom slash commands and skills, applying path-specific rules, choosing plan mode vs direct execution, applying iterative refinement techniques, and integrating Claude Code into CI/CD pipelines.
  Task statements:
  ...
  Bullet count for this domain: 49 (Task 3.1: 8, 3.2: 9, 3.3: 6, 3.4: 8, 3.5: 9, 3.6: 9)

- Domain: Prompt Engineering & Structured Output (20%)
  Description: Covers designing prompts with explicit criteria to reduce false positives, applying few-shot prompting, enforcing structured output via tool_use and JSON schemas, implementing validation/retry/feedback loops, designing batch processing strategies, and designing multi-instance/multi-pass review architectures.
  Task statements:
  ...
  Bullet count for this domain: 47 (Task 4.1: 6, 4.2: 9, 4.3: 10, 4.4: 8, 4.5: 8, 4.6: 6)

- Domain: Context Management & Reliability (15%)
  Description: Covers managing conversation context to preserve critical information over long interactions, designing escalation and ambiguity-resolution patterns, implementing error propagation across multi-agent systems, managing context in large codebase exploration, designing human review/confidence-calibration workflows, and preserving information provenance in multi-source synthesis.
  Task statements:
  ...
  Bullet count for this domain: 53 (Task 5.1: 10, 5.2: 9, 5.3: 8, 5.4: 9, 5.5: 8, 5.6: 9)
...
- Domains: 5
- Task statements: 30 (Domain 1: 7, Domain 2: 5, Domain 3: 6, Domain 4: 6, Domain 5: 6)
- Total bullets captured (Knowledge of + Skills in, across all task statements): 240
  (Domain 1: 48, Domain 2: 43, Domain 3: 49, Domain 4: 47, Domain 5: 53)
