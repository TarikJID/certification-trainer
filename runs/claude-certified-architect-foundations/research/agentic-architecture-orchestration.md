...
- Concept: MCP tool results and the `isError` pattern
  ...
  Definition: Tools exposed through the Model Context Protocol (MCP) return results as content blocks that can carry an `isError` flag and arbitrary payload shapes chosen by the tool author. Because different backend systems format the same kind of data differently (a timestamp as Unix epoch seconds versus ISO 8601, a status as a numeric code versus a string), the raw tool output an agent receives from one MCP server is frequently inconsistent with the output of another, even when both represent the same underlying concept. This heterogeneity is what makes a normalization step necessary before the model reasons over the combined results.
  Example: An `orders` MCP server returns `{"status_code": 3}` for "shipped" while a `billing` MCP server returns `{"status": "SHIPPED"}` for the same concept; a consuming agent that treats both as directly comparable will misclassify one of them.
  Source: https://code.claude.com/docs/en/mcp (MCP tool results/isError conventions), corroborated by the exam guide's Appendix technologies list ("isError flag") — runs/claude-certified-architect-foundations/sources/exam-guide.txt, line 976
