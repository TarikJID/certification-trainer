...
Note on sourcing: the official exam guide PDF could not be parsed as text by the available fetch tool across repeated attempts (it consistently returns raw compressed/FlateDecode PDF stream data instead of extractable text). For this revision, the orchestrator relayed verbatim task-statement quotes from Domain 5 (Tasks 5.1, 5.2, 5.5) of the guide, obtained via the evaluator's own successful parse of the PDF. Three concepts below (marked explicitly) are therefore sourced to the exam guide itself, which is explicitly permitted as "official certification material" under this task's quality bar, rather than to Anthropic vendor documentation — because a genuine search for corroborating vendor documentation on those specific sub-topics turned up nothing beyond third-party exam-prep aggregator sites, which are excluded by the quality bar and were not used. Where a specific sub-topic named in the domain description or exam guide does not have a dedicated, named treatment in Anthropic's own product/engineering documentation, this is flagged explicitly rather than filled in from memory.
