...
- Concept: Test-driven iteration
  ...
  Definition: Writing a test suite that covers expected behavior, edge cases, and performance requirements before implementation gives an unambiguous, machine-checkable definition of "done." The recommended workflow is: write the tests, confirm they fail, optionally commit the failing tests as a checkpoint, then have Claude implement until they pass. Each red-to-green cycle is direct feedback Claude can iterate on without waiting for a human to manually verify correctness after every change; sharing test failures on each iteration is what drives progressive improvement.
  Example: Before asking Claude to implement a rate limiter, a developer writes tests asserting the limiter rejects the 11th request in a 10-request-per-minute window and allows the 1st after the window resets; Claude implements against those tests and iterates until the suite is green.
  Source: https://code.claude.com/docs/en/best-practices ("Give Claude a way to verify its work" — tests as a check Claude can run and iterate against until it passes)
