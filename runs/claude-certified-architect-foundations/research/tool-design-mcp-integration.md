...
- Concept: Tool description ambiguity/overlap and the rename/split fix
  ...
  Definition: When two tools have near-identical descriptions (e.g., `analyze_content` vs `analyze_document`), the model cannot reliably tell which one applies to a given request and misroutes — calling the wrong tool or alternating unpredictably between them. Anthropic's engineering team found this directly through internal testing: "Bad tool descriptions can send agents down completely wrong paths, so each tool needs a distinct purpose and a clear description," and building a tool-testing agent to rewrite ambiguous descriptions produced a measured 40% decrease in task completion time. Two concrete repair techniques: (a) rename a tool and rewrite its description to be domain-specific instead of generic (e.g., `analyze_content` → `extract_web_results`, scoped explicitly to web content) so its name/description no longer overlaps with a neighboring tool; (b) split an overly generic tool (`analyze_document`) into several purpose-specific tools (`extract_data_points`, `summarize_content`, `verify_claim_against_source`), each with its own narrow, well-defined input/output contract, instead of cramming multiple use cases behind one ambiguous interface.
  ...
  Source: exam guide 2.1-K3, 2.1-S2, 2.1-S3 (task statement's own wording); https://www.anthropic.com/engineering/writing-tools-for-agents (namespacing/consolidation guidance and the tool-testing-agent finding).
...
- Concept: Simultaneous discovery of tools across all configured MCP servers
  Type: key
  Teaches: 2.4-K3
  Definition: When a Claude Code session starts, it connects to every MCP server configured across the applicable scopes (local, project, user, plugin-provided, etc.) and, for each connected server, sends a `tools/list` request to discover that server's tools. All discovered tools from all connected servers are then made available to the agent simultaneously within the same session — the agent is not limited to tools from one server at a time and does not need to explicitly switch between servers. Tool names are namespaced by server (and plugin, if applicable) to avoid collisions when multiple servers expose similarly named tools.
  Example: A developer with both a Jira MCP server (project scope) and a personal database MCP server (user scope) configured sees tools from both — e.g., `mcp__jira__create_issue` and `mcp__postgres__query` — available together in the same session, without activating one server over the other.
  Source: https://code.claude.com/docs/en/mcp (Tool Discovery and Availability); MCP `tools/list` protocol behavior at https://modelcontextprotocol.io/specification/2025-06-18/server/tools.
